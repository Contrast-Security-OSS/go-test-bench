...
func (o *PathTraversalGetQueryOpenURL) Build() (*url.URL, error) {
	...
	var _path = "/pathTraversal/os.Open/query/{safety}"

	safety := o.Safety
	if safety != "" {
		_path = strings.Replace(_path, "{safety}", safety, -1)
	} else {
		return nil, errors.New("safety is required on PathTraversalGetQueryOpenURL")
	}
...
func (o *PathTraversalGetQueryOpenURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PathTraversalGetQueryOpenURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PathTraversalGetQueryOpenURL")
	}
