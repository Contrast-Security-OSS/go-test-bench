...
func (o *PathTraversalGetBufferedQueryWriteFileURL) Build() (*url.URL, error) {
	...
	var _path = "/pathTraversal/os.WriteFile/buffered-query/{safety}"

	safety := o.Safety
	if safety != "" {
		_path = strings.Replace(_path, "{safety}", safety, -1)
	} else {
		return nil, errors.New("safety is required on PathTraversalGetBufferedQueryWriteFileURL")
	}
...
func (o *PathTraversalGetBufferedQueryWriteFileURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on PathTraversalGetBufferedQueryWriteFileURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on PathTraversalGetBufferedQueryWriteFileURL")
	}
